import {
  ...
  export const allocationChecker = async (
    ...
    try {
      ...
      if (mintCounter) {
        ...
        console.log("Allocation Guard not Initialized! Minting will fail!");
  ...
  export const calculateMintable = (
    ...
    if (!import.meta.env.VITE_MAXMINTAMOUNT) return mintableAmount;
    let maxmintamount = 0;
    try {
      maxmintamount = Number(import.meta.env.VITE_MAXMINTAMOUNT);
    } catch (e) {
      console.error("process.env.VITE_MAXMINTAMOUNT is not a number!", e);
